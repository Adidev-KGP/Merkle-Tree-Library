...
TEST test_SHA256(void) {
    ...
    for(int i=0;i<2;i++){
        ...
        for(int j=0;j<32;j++){
            ...
            ASSERT_EQm("OOPs x doesn't equal 1", data_hash_check[i][j], data_hash[i][j]);
...
TEST test_element_hash(void) {
    ...
    for(int i=0;i<2;i++){
        ...
        for(int j=0;j<32;j++){
            ...
            /* Compare, with a custom failure message */
            ASSERT_EQm("OOPs x doesn't equal 1", ele_hash_check[i][j], ele_hash[i][j]);
...
TEST test_combine_hashes(void) {
   ...
   byte com_hash_check[32]={
    ...
    for(int i=0;i<32;i++){
        ...
        /* Compare, with a custom failure message */
        ASSERT_EQm("OOPs x doesn't equal 1", com_hash_check[i], com_hash[i]);
