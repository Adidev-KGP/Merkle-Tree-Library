...
TEST test_hexstr_to_num(void) {
    ...
    for(int i=0;i<10;i++){
        ...
        ASSERT_EQm("OOPs x doesn't equal 1", byte_ele[i], x);
...
TEST test_hexstr_to_data(void) {
    ...
    for(int i=0;i<32;i++){
        ...
        /* Compare, with a custom failure message */
        ASSERT_EQm("Yikes, x doesn't equal 1", hash_byte_check[i], hash_byte[i]);
...
TEST test_final_processed_data(void) {
    ...
    for(int i=0;i<3;i++){
        for(int j=0;j<32;j++){
            ...
            /* Compare, with a custom failure message */
            ASSERT_EQm("Yikes, x doesn't equal 1", hash_byte_check[i][j], hash_byte[i][j]);
